...
if [ $1 = "little" ]; then
	...
	do 
	   ...
	   cat temp3 | awk '{printf "%.6f,%.6f,%.6f,%.6f,%.6f\n", $1, $2, $3, $4, $5}' > $pmcs".post_process"	
	...
	do  
	   ...
	   cat temp3 | awk '{printf "%.6f,%.6f,%.6f,%.6f,%.6f,%.6f,%.6f\n", $1, $2, $3, $4, $5, $6, $7}' > $pmcs".post_process"
	...
	sed -i -n "1,$NUM_LINHAS_COMUM p" *.post_process
        ...
        cut -d, -f1,5,11,16,21,26,31,36,41,46 --complement $OUTPUT_FILE_NAME > temp
        mv temp $OUTPUT_FILE_NAME
...
if [ $1 = "little" ]; then
   ...
   cat $OUTPUT_FILE_NAME | tr "," "\t" | tr "." "," | datamash mean 1-34 | tr "," "." | tr "\t" "," > $OUTPUT_FILE_NAME".average"
   ...
   cat $OUTPUT_FILE_NAME | tr "," "\t" | tr "." "," | datamash mean 1-57 | tr "," "." | tr "\t" "," > $OUTPUT_FILE_NAME".average"
   #paste temp exec_time.average -d "," >  $OUTPUT_FILE_NAME".average"
fi
